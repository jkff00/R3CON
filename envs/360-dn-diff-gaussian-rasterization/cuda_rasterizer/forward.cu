	...
	bool prefiltered)
{
  ...
  else
  {
	...
	// Compute 2D screen-space covariance matrix
  //add
  // float phi   = p_proj.y * (M_PI * 0.5f);
  // float cosphi = max(cosf(phi),sinf(M_PI / (2.0f * H)));
  // float fx_equiv = W/(2*M_PI*cosphi);  // like W / (2π cosφ)
  // float fy_equiv = H / M_PI;
	// float3 cov = computesphericalCov2D(p_orig, fx_equiv, fy_equiv, tan_fovx, tan_fovy, cov3D, viewmatrix);

  float3 t = transformPoint4x3(p_orig, viewmatrix);
  
  float t_length = sqrtf(t.x * t.x + t.y * t.y + t.z * t.z);
	
	float my_lat = p_view.y * M_PI / 2;
	float my_lon = p_view.x * M_PI;
  
  float3 cov = computeOmniCov2D(my_lon, my_lat, t_length, cov3D, viewmatrix, H, W);
...
//   uint32_t* __restrict__ n_contrib,jxf
  ...
  int* __restrict__ pixel_contrib,
  float visible_thresh)//add jxf
{
...
//   float dist_min = FLT_MAX;
  ...
  for (int i = 0; i < rounds; i++, toDo -= BLOCK_SIZE)
  {
    ...
    for (int j = 0; !done && j < min(BLOCK_SIZE, toDo); j++)
    {
      ...
      if(last_contributor==0 )//add jxf
      {
        float dist2 = d.x * d.x + d.y * d.y;
     
        float gz = depths[collected_id[j]];
        if (dist2*gz*gz>visible_thresh)
          continue;

        last_contributor = collected_id[j];
        pixel[0] = int(collected_xy[j].x + 0.5f); 
        pixel[1] = int(collected_xy[j].y + 0.5f); 
      }

//       last_contributor = collected_id[j];//jxf
    }
  }
  ...
  if (inside)
  {
    ...
    //add jxf
    pixel_contrib[pix_id * 2] = pixel[0];     // x
    pixel_contrib[pix_id * 2 + 1] = pixel[1]; // y
...
//   uint32_t* n_contrib,jxf
  ...
  float* out_extra,
  int* pixel_contrib,
  const float visible_thresh)//add jxf
{
