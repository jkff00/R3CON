...
def rasterize_gaussians(
    ...
    color, depth, norm, alpha, radii, extra, contrib,pixel_contrib = _RasterizeGaussians.apply(
        means3D,
        means2D,
        sh,
        colors_precomp,
    ...
    # 3, H, W
    return color, depth, norm, alpha, radii, extra, contrib,pixel_contrib
...
class _RasterizeGaussians(torch.autograd.Function):
    ...
    def forward(
        ...
        args = (
            ...
            raster_settings.visible_thresh#add jxf
        )
        #add jxf
        contrib = None
        pixel_contrib = None
        ...
        if raster_settings.debug:
            ...
            try:
                if raster_settings.spherical:
                    # num_rendered, color, depth, norm, alpha, radii, extra, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_spherical_gaussians(*args)
                    num_rendered, color, depth, norm, alpha, radii, extra, contrib, pixel_contrib, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_spherical_gaussians(*args)
                else:
                    num_rendered, color, depth, norm, alpha, radii, extra,contrib,pixel_contrib, geomBuffer, binningBuffer, imgBuffer = _C.rasterize_gaussians(*args)#add jxf
            except Exception as ex:
                torch.save(cpu_args, "snapshot_fw.dump")
                print("\nAn error occured in forward. Please forward snapshot_fw.dump for debugging.")
                raise ex
    ...
    def backward(ctx, grad_out_color, grad_out_depth, grad_out_norm, grad_out_alpha, _, grad_out_extra):
        ...
        if raster_settings.debug:
            ...
            try:
                if raster_settings.spherical:
                    grad_means2D, grad_colors_precomp, grad_opacities, grad_means3D, grad_cov3Ds_precomp, grad_norm3Ds_precomp, grad_sh, grad_scales, grad_rotations, grad_extra_attrs = _C.rasterize_spherical_gaussians_backward(*args)
                else:
