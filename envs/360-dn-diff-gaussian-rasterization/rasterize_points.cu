...
//add jxf
  torch::Tensor out_contrib = torch::full({H*W}, 0, int_opts);
  torch::Tensor pixel_contrib = torch::full({H*W,2}, 0, int_opts);
  //add jxf
  ...
  if(P != 0)
  {
    ...
    rendered = CudaRasterizer::Rasterizer::forward(
      ...
      out_contrib.contiguous().data<int>(),
      pixel_contrib.contiguous().data<int>(),
      visible_thresh,
      debug) ;//add jxf
  }
  return std::make_tuple(rendered, out_color, out_depth, out_norm, out_alpha, radii, out_extra,out_contrib,pixel_contrib,geomBuffer, binningBuffer, imgBuffer);//add jxf
